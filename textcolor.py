...
def render_color_text(text, ws):
    ...
    min_ws = np.min(ws)
    max_ws = np.max(ws)
    d = max_ws - min_ws if max_ws != min_ws else 1.0
    ws = (ws - min_ws) / d * 0.99
...
def print_color_text(text, ws):
    ws = np.array(ws)
    ws = (ws - np.min(ws)) / (np.max(ws) - np.min(ws) + 0.05) * 0.99
    for string, w in zip(text, ws):
        vid = int(w * length)
        value = ansi_code_ids[vid]
...
def print_match_subsequence(text1, text2, printed=True, reverse_render=False):
    # 高亮两个文本的最长公共子序列（LCS）
    if not (text1 and text2):
        return
    s1 = len(text1)
...
if __name__ == "__main__":
    ...
    ansi_code_ids.extend([34, 32])
    length = len(ansi_code_ids)
    print_color_text(text[:length], np.arange(length))
